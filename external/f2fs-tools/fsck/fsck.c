		...
		u32 *blk_cnt, struct node_info *ni, struct child_info *child_d)
{
	...
	for (idx = 0; idx < 5; idx++) {
		...
		if (_nid != 0 && IS_VALID_NID(sbi, _nid)) {
...
skip_blkcnt_fix:
	namelen = (int)le32_to_cpu(node_blk->i.i_namelen);
	...
	namelen = convert_encrypted_name(node_blk->i.i_name, namelen,
					en, file_enc_name(&node_blk->i)); /*lint !e713*/
	en[namelen] = '\0';
	if (ftype == F2FS_FT_ORPHAN)
...
u32 convert_encrypted_name(unsigned char *name, int len,
		...
		int max, int idx, int last_blk, int enc_name)
{
	...
	convert_encrypted_name(name, (unsigned int)name_len, new, enc_name);
...
static void __fix_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	/* if has append nat block,we need set append flag
	 * and sit count is nullified,no need to set sit flag
	 */
	...
	*((__le32 *)((unsigned char *)cp + CHECKSUM_OFFSET)) = cpu_to_le32(crc);
	...
	if (flags & CP_APPEND_NAT_FLAG) {
		...
		block_t start_blk =  __start_cp_addr(sbi) +
