...
void print_inode_info(struct f2fs_inode *inode, int name)
{
	...
	for (i = 4; i < ADDRS_PER_INODE(inode); i++) {
		if (inode->i_addr[i] != 0x0) {
			MSG(0, "i_addr[0x%x] points data block\r\t\t[0x%4x]\n",
...
int sanity_check_raw_super(struct f2fs_super_block *sb, u64 offset)
{
	...
	if (!crc) {
		MSG(0, "crc is zero, this partition may not support crc");
	} else {
		if (f2fs_crc_valid(crc, sb, offsetof(struct f2fs_super_block, crc))) {
...
int validate_super_block(struct f2fs_sb_info *sbi, int block)
{
	...
	if (!sanity_check_raw_super(sbi->raw_super, offset)) {
		/* get kernel version */
		if (c.kd >= 0) {
			if (dev_read_version(c.version, 0, VERSION_LEN) < 0)
			{
				MSG(0, "\tError: While reading config version\n");
				goto free;
			}
		...
		if (cmp_kernel_version(c.sb_version, c.version)) {
			...
			/* The very first booting right after android is
			 * loaded to device, the sb_version is empty. We
			 * check it to see if fsck is running in this
			 * situation. If it is, we do not report errors
			 * to DMD.
			 */
			if (c.sb_version[0] == '\0')
				fsck->is_first_fsck = 2;
			...
			/* recalculate checksum because we update the versions in superblock */
			old_crc = le32_to_cpu(sbi->raw_super->crc);
			if (old_crc) {
				crc = f2fs_cal_crc32(F2FS_SUPER_MAGIC, (unsigned char*)(sbi->raw_super), offsetof(struct f2fs_super_block, crc));
...
free:
	...
	/* here we cannot get a valid superblock because of the corrupted
	 * magic number, sb crc, size. The reason may be FS encryption. So
	 * we should skip this fsck
	 */
	fsck->is_first_fsck++;
	return -EINVAL;
}
...
void clear_extra_flag(struct f2fs_sb_info *sbi, unsigned int flag)
{
	...
	ret = dev_write_block(ef_blk, cp_blkaddr + sbi->blocks_per_seg - 1);
	if (ret < 0)
...
void *validate_checkpoint(struct f2fs_sb_info *sbi, u64 cp_addr,
...
void check_sit_nat_journal(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < sits_in_cursum(sit_journal); i++) {
		...
		if (segno > (TOTAL_SEGS(sbi) - 1)) {
			ASSERT_MSG("Invalid SEGNO: 0x%x", segno);
			segno_in_journal(sit_journal, i) = 0;
			memset(&sit_in_journal(sit_journal, i), 0,
				sizeof(struct f2fs_sit_entry));
			FIX_MSG("Remove sit [0x%x] in sit journal\n", segno);
		}
	}
...
void __write_checkpoint(struct f2fs_sb_info *sbi)
{
	...
	*((__le32 *)((unsigned char *)cp + CHECKSUM_OFFSET)) = cpu_to_le32(crc);
	...
	if (flags & CP_APPEND_NAT_FLAG) {
		struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_HOT_DATA);
		block_t nat_blk =  __start_cp_addr(sbi) +
			le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count);
		ret = dev_write_block((char *)curseg->journal + SUM_JOURNAL_SIZE - NAT_JOURNAL_RESERVED,
					nat_blk);
		nat_block = 1;
		ASSERT(ret >= 0);
	}
	...
	if (flags & CP_APPEND_SIT_FLAG) {
		struct curseg_info *curseg = CURSEG_I(sbi, CURSEG_COLD_DATA);
		block_t sit_blk =  __start_cp_addr(sbi) +
			le32_to_cpu(F2FS_CKPT(sbi)->cp_pack_total_block_count) + nat_block;
		ret = dev_write_block((char *)curseg->journal + SUM_JOURNAL_SIZE - SIT_JOURNAL_RESERVED,
			sit_blk);
		ASSERT(ret >= 0);
	}
	/* write the last cp */
...
static int check_sector_size(struct f2fs_sb_info *sbi)
{
	int index;
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	...
	old_crc = get_sb(crc);
	new_crc = f2fs_cal_crc32(F2FS_SUPER_MAGIC, (unsigned char*)(sbi->raw_super), offsetof(struct f2fs_super_block, crc));
