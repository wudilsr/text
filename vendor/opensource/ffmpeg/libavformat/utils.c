...
static uint32_t get_aac_sample_rate(AVFormatContext *ic, int64_t offset)
{
    int sf_index;
    char head[9] = {0};
    ...
    avio_seek(ic->pb, offset, SEEK_SET);
    avio_read(ic->pb, head, 9);

    if (head[0] != 0xff || (head[1] & 0xf6) != 0xf0)
        return 0;

    sf_index = (head[2] >> 2) & 0xf;
...
static int64_t get_adts_frame_length(AVIOContext *s, int *isTagFrame)
{
    int64_t frameSize = 0;
    char head[9] = {0};
    ...
    frameSize = (head[3] & 0x3) << 11 | head[4] << 3 | head[5] >> 5;

    return frameSize;
}

static uint32_t get_aac_frames_num(AVFormatContext *ic, int64_t offset)
{
    int64_t frameSize, file_size, pos = offset;
    uint32_t numFrames = 0;
    int isTagFrame = 0;

    file_size = FFMAX(0, avio_size(ic->pb));

    avio_seek(ic->pb, pos, SEEK_SET);
    while (pos < file_size) {
        if ((frameSize = get_adts_frame_length(ic->pb, &isTagFrame)) == 0)
            return 0;
...
static size_t get_ID3V2_length(AVFormatContext *ic)
{
    size_t len;
    char ID3V2_head[ID3v2_HEADER_SIZE] = {0};

    avio_seek(ic->pb, 0, SEEK_SET);
    for(;;) {
        if (avio_read(ic->pb, ID3V2_head, ID3v2_HEADER_SIZE) < ID3v2_HEADER_SIZE)
            return 0;

        if (ID3V2_head[0] != 'I' || ID3V2_head[1] != 'D' || ID3V2_head[2] != '3')
            break;

        len = ((ID3V2_head[6] & 0x7f) << 21)
                | ((ID3V2_head[7] & 0x7f) << 14)
                | ((ID3V2_head[8] & 0x7f) << 7)
                | (ID3V2_head[9] & 0x7f);
...
static void estimate_timings_for_aac(AVFormatContext *ic)
{
    uint32_t sr, numFrames;
    int64_t cur, offset, mFrameDurationUs, duration;

    cur    = avio_tell(ic->pb);
    offset = get_ID3V2_length(ic);

    sr = get_aac_sample_rate(ic, offset);
    if(0 == sr)
        goto end;

    numFrames = get_aac_frames_num(ic, offset);
    if(0 == numFrames)
        goto end;
    ...
    for (int i = 0; i < ic->nb_streams; i++) {
        if (ic->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            ic->streams[i]->duration = av_rescale(duration, ic->streams[i]->time_base.den,
                                       (int64_t)ic->streams[i]->time_base.num)/AV_TIME_BASE;
            ic->duration_estimation_method = AVFMT_DURATION_FROM_USER;
            avio_seek(ic->pb, cur, SEEK_SET);
            return;
        } else {
            continue;
        }
...
static void estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    } else if (!strcmp(ic->iformat->name, "aac")) {
        estimate_timings_for_aac(ic);
    } else {
...
int unpack_hevc_ps(AVCodecParameters *codecpar, AVPacket *pkt, int* find_hevc_ps)
{
    int i, iOffset=0;
        ...
        ||(state4&0xFFFFFF7E) == 0x144)
    {
        if (codecpar->extradata == NULL)
        {
            ...
            codecpar->extradata      = av_malloc(iStreamLen+5);
            codecpar->extradata_size = iStreamLen+5;
        }
        else
        {
             pExtradataTmp = av_realloc(codecpar->extradata, iStreamLen+5+codecpar->extradata_size);
             iOffset = codecpar->extradata_size;

             codecpar->extradata = pExtradataTmp;
             codecpar->extradata_size += iStreamLen+5;
        }

        ptr = codecpar->extradata;
        ptr[22]++;
...
{
    AVCodecParameters *codecpar = ic->streams[pkt->stream_index]->codecpar;
    int i, iStart=0;
    ...
    if (ret == 0)
    {
        if (NULL != codecpar->extradata)
        {
            pktTmp.size = codecpar->extradata_size;
            pktTmp.data = av_malloc(codecpar->extradata_size);
            memcpy(pktTmp.data, codecpar->extradata, codecpar->extradata_size);
        ...
        ptr = (uint8_t *)codecpar->extradata;
        ...
        ret = unpack_hevc_ps(codecpar, &pktTmp, &ret);
    }

    ret = unpack_hevc_ps(codecpar, pkt, &ret);
...
int k3_find_stream_info(AVFormatContext *ic, int tryDecodeNumber)
{
    ...
    int i, count = 0, ret = 0, j, k3_count = 0;
    int find_hevc_ps = 0;
    int64_t read_size;
    AVStream *st;
    AVCodecContext *avctx;
    AVPacket pkt1, *pkt = NULL;
    ...
    if (!strncmp(ic->iformat->name, "mpegts", 6) && !strncmp(ic->filename, "file://", 7)) {
        probesize *= 2;
    }
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    //for(;;) {
        ...
        if ((AV_CODEC_ID_HEVC == ic->streams[pkt1.stream_index]->codecpar->codec_id)
            && !strncmp(ic->iformat->name, "flv", 3))
        {
            if ( 0xF != find_hevc_ps)
            {
    ...
    if (pkt && !strncmp(ic->iformat->name, "flv", 3)
            && AV_CODEC_ID_HEVC == ic->streams[pkt->stream_index]->codecpar->codec_id)
    {
        ret = avcodec_parameters_to_context(ic->streams[pkt->stream_index]->internal->avctx, ic->streams[pkt->stream_index]->codecpar);
        if (ret < 0)
            goto find_stream_info_err;
    }
...
static const product_cust kQcom_product_cust_list[] = {
    ...
    {   "MLA-AL..$|MLA-UL..$|MLA-TL..$|CAZ-TL..$|CAZ-AL..$",
        (const enum AVCodecID [])
        {QCOM_MIN_SET, AV_CODEC_ID_MPEG1VIDEO,AV_CODEC_ID_MJPEG,AV_CODEC_ID_APE,AV_CODEC_ID_WMAV2,AV_CODEC_ID_MSS2,AV_CODEC_ID_WMV1,AV_CODEC_ID_WMV2,AV_CODEC_ID_WMV3},
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]) + 8
    },
    {
        "H1711",
        (const enum AVCodecID [])
        {QCOM_MIN_SET, AV_CODEC_ID_EVRC, AV_CODEC_ID_QCELP},
        sizeof(kQcom_min_set)/sizeof(kQcom_min_set[0]) + 2
    },
};
...
static int av_is_codecid_supported(enum AVCodecID codec_id)
{
    ...
    if (!strncmp(prop_platform, "hi", 2) || !strncmp(prop_platform, "kirin", 5)) {
        return 1;
    } else {
        return 0;
    }
                        ...
                        AVCodec **decoder_ret, int flags)
{
    ...
    for (i = 0; i < nb_streams; i++) {
        ...
        if (!av_is_codecid_supported(par->codec_id))
            continue;

        char strProp[PROPERTY_VALUE_MAX] = {0};
        property_get("ro.config.dolby_ddp", strProp, "false");
        if (strncmp("true", strProp, sizeof(strProp))) {
            if((par->codec_id == AV_CODEC_ID_AC3) || (par->codec_id == AV_CODEC_ID_EAC3)) {
                av_log(ic, AV_LOG_ERROR, "We can't support %s(0x%08x)", avcodec_get_name(par->codec_id), par->codec_id);
                continue;
            }
        }
        if((par->codec_id == AV_CODEC_ID_DTS) || (par->codec_id == AV_CODEC_ID_RAWVIDEO)) {
            av_log(ic, AV_LOG_ERROR, "We can't support %s(0x%08x)", avcodec_get_name(par->codec_id), par->codec_id);
            continue;
        }
