...
/* add buf_end to limit the range of traverse, becasue sometimes the size in the tag(packet) header is fake or the data of the tag(packet) body is truncated*/
static int hevc_find_frame_type(unsigned char *buf_ptr, int size, int frame_type, const unsigned char *buf_end)
{
    int i = 0;
    int state = 0xffffffff;

    if (NULL == buf_ptr || size <= 4 || NULL == buf_end)
    {
        return frame_type;
    }

    for (i = 0; i < size && buf_ptr + i < buf_end; i++)
    {
        if( (state & 0xFFFFFF) == 0x1)
        {
            state = (state << 8) | buf_ptr[i];

            /*find a vps, sps, pps*/
            if ((state & 0x7E) == 0x40
                    || (state & 0x7E) == 0x42
                    || (state & 0x7E) == 0x44
                    || (((state & 0x7E) >= 0x20) && ((state & 0x7E) <= 0x2E)))
            {
                return FLV_FRAME_KEY;
            }
            else
            {
                return FLV_FRAME_INTER;
            }
        }
        state = (state << 8) | buf_ptr[i];
    }
...
static int find_next_tag_header(AVFormatContext *s, uint32_t *ts, int64_t fsize)
{
    uint32_t in_ts = 0;
    int size       = 0;
    int pre_size   = 0;

    // check flv tag type is legal
    int type = avio_r8(s->pb);
    if ((type != 18)&&(type != 8)&&(type != 9)) {
        return -1;
    }

    size = avio_rb24(s->pb);
    if ((size > fsize) || (size < 0)) {
        av_log(s, AV_LOG_WARNING,"size: %d sorry !\n", size);
        return -1;
    }

    // get timestamp from current tag struct
    in_ts  = avio_rb24(s->pb);
    in_ts |= avio_r8(s->pb) << 24;

    // skip strem id, 3 byte
    avio_seek(s->pb, size+3, SEEK_CUR);
    pre_size = avio_rb32(s->pb);
    if (pre_size == (size+11)) {
      *ts = in_ts;
      return 0;
    }
...
static int  find_last_timestamp(AVFormatContext *s, uint32_t *timestamp)
{
    uint32_t ts    = 0;
    int64_t  fsize = avio_size(s->pb);

    // skip flv file header, 13 byte
    avio_seek(s->pb, 13, SEEK_SET);

    while (find_next_tag_header(s, &ts, fsize) == 0);
...
static void get_duration_by_last_tag(AVFormatContext *s)
{
    uint32_t ts = 0;
    find_last_timestamp(s, &ts);

    if (ts) {
        s->duration = ts * (int64_t)AV_TIME_BASE / 1000;
        av_log(s, AV_LOG_WARNING,"[%s] duration : %lld \n",__func__, s->duration);
    }
}
...
retry:
    /* pkt size is repeated at end. skip it */
        ...
        if (type == FLV_TAG_TYPE_AUDIO) {
            ...
            {
                if (FLV_CODECID_HEVC == (flags & FLV_VIDEO_CODECID_MASK))
                {
                    frame_type = flags & FLV_VIDEO_FRAMETYPE_MASK;
                    frame_type = hevc_find_frame_type(s->pb->buf_ptr, size, frame_type, s->pb->buf_end);
                    flags = (frame_type & FLV_VIDEO_FRAMETYPE_MASK) + (flags & FLV_VIDEO_CODECID_MASK);
                }
            }
