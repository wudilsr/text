...
int av_decode(AVCodecContext *avctx, AVFrame *frame, int *got_frame, const AVPacket *pkt)
{
    int ret;
    ...
    *got_frame = 0;
    ...
    if (pkt) {
        ...
        if (ret < 0 && ret != AVERROR_EOF) {
            av_log(avctx,AV_LOG_ERROR,"avcodec_send_packet fail");
            return ret;
        }
    }
    ...
    if (ret < 0 && ret != AVERROR(EAGAIN)) {
        av_log(avctx,AV_LOG_ERROR,"avcodec_receive_frame fail");
        return ret;
    }
    if (ret >= 0)
        *got_frame = 1;
...
int k3_parameters_to_context(AVCodecContext *codec,
                                  const AVCodecParameters *par)
{
    codec->codec_type = par->codec_type;
    codec->codec_id   = par->codec_id;
