 ...
 */
#define workingset_dealwith_range_header_bit(entry, start, end, major, delta) \
{	\
...
#define workingset_handle_range_insert_result(root, entry, merged_entry, \
	major_touched, parent, repeat_pages, \
...
static struct s_pagecache_info *workingset_range_rb_insert(
	...
	int major_touched, unsigned *repeat_pages, int *page_count_delta)
{
	...
	workingset_handle_range_insert_result(root, entry, merged_entry,
		major_touched, parent, repeat_pages, page_count_delta,
		repeat_cnt, page_delta);
...
static void workingset_writeback_last_record_if_need(void)
{
	...
	if (g_record_cnt >= MAX_WORKINGSET_RECORDS) {
 ...
 * prefer to new a record when the count cached records is small than
 * MAX_WORKINGSET_RECORDS, or select a cached record by lru.
 */
static struct s_ws_record *workingset_get_available_record(void)
{
	...
	if (g_record_cnt < MAX_WORKINGSET_RECORDS) {
		...
		if (record) {
			...
			spin_unlock(&g_record_list_lock);
			pr_warn("%s, no available record can be replaced!\n",
				__func__);
		}
	}
	...
	struct s_ws_data *data, unsigned playload_pages, unsigned *pidx)
{
	...
	if (data->array_page_cnt)
		memcpy(page_array, data->page_array,
			sizeof(struct page *) * data->array_page_cnt);
	...
	struct s_ws_owner *owner)
{
	...
	memcpy(record->owner.record_path, owner->record_path,
		strlen(owner->record_path) + 1);
	data = &record->data;
	...
	if (target_state == E_CGROUP_STATE_MONITOR_PREREAD
		...
		if (record)  {
			...
			&& !(record->state & E_RECORD_STATE_PREREADING)) {
				...
				E_CGROUP_STATE_MONITOR_STOP) {
				workingset_preread_post_work_rcrdlocked(
					record);
				record->state &= ~E_RECORD_STATE_PREREADING;
			} else if (target_state ==
				E_CGROUP_STATE_MONITOR_ABORT) {
	...
	char *owner_string)
{
	...
	owner_name = kzalloc(len + 1, GFP_NOFS);
	if (!owner_name) {
		ret = -ENOMEM;
		goto out;
	}
	...
	record_path = kzalloc(len + 1, GFP_NOFS);
	if (!record_path) {
		ret = -ENOMEM;
		goto parse_path_failed;
	}
	...
	struct file *file, struct s_file_info **file_info, bool *is_existed)
{
	...
	if (!existed) {
		...
		strncpy(fileinfo->path_node.path, filepath, path_len);
	...
	struct s_workingset *ws)
{
	...
	if (workingset_range_rb_insert(&file_info->rbroot, pagecache,
		major_touched, &repeat_count, &page_count_delta)) {
	...
	struct s_ws_collector *collector, char *buffer, size_t buf_size)
{
	...
	if (read_pos > write_pos) {
		...
		memcpy(buffer, collector->circle_buffer + read_pos,
			copy_size);
		read_pos += copy_size;
...
static void workingset_collector_fill_record_filenode_locked(
	...
	list_for_each(pos, head) {
		...
		if (idx < data->file_cnt) {
			if (!fileinfo->pageseq_count) {
				kfree(fileinfo->path_node.path);
				memset(data->file_array + idx,
					0, sizeof(struct s_path_node));
			} else {
				memcpy(data->file_array + idx,
					&fileinfo->path_node,
	...
	struct s_ws_record *record, bool is_exist)
{
	...
	if (!is_exist) {
		...
		memcpy(record->owner.name, ws->owner.name,
			strlen(ws->owner.name) + 1);
		memcpy(record->owner.record_path, ws->owner.record_path,
			strlen(ws->owner.record_path) + 1);
	}
	data->file_cnt = ws->file_count;
	data->pageseq_cnt = ws->pageseq_count;
	data->page_sum = ws->page_sum;
	workingset_collector_fill_record_filenode_locked(data, &ws->file_list);
...
static bool workingset_filepage_is_need_skip_read(struct s_ws_data *data,
	unsigned idx, bool preread_major, struct file **filpp,
	unsigned *pfile_idx)
{
	bool ret = false;
	bool is_major_page;
...
try_next:
	for (idx = 0; idx < data->pageseq_cnt; idx++) {
		if (!(idx%100) && atomic_read(&g_preread_abort))
			return;
		if (workingset_filepage_is_need_skip_read(data, idx,
			preread_major_page, filpp, &file_idx))
			continue;

		if (!filpp[file_idx]) {
			filpp[file_idx] = filp_open(
				data->file_array[file_idx].path, flags, 0);
...
static void workingset_do_preread_work_rcrdlocked(struct s_ws_record *record)
{
	...
	record->preread_filpp = filpp;
	workingset_read_filepages_rcrdlocked(record, filpp);
}

static void workingset_preread_post_work_rcrdlocked(struct s_ws_record *record)
{
	unsigned file_idx, idx;
	struct s_ws_data *data;

	data = &record->data;
	if (record->preread_filpp) {
		for (file_idx = 0; file_idx < data->file_cnt; file_idx++) {
			if (record->preread_filpp[file_idx])
				filp_close(record->preread_filpp[file_idx],
						NULL);
		}
	}
	...
	for (idx = 0; idx < FILP_PAGES_COUNT; idx++) {
		if (record->filp_pages[idx]) {
			if (record->preread_filpp) {
				vunmap(record->preread_filpp);
				record->preread_filpp = NULL;
			}
			__free_page(record->filp_pages[idx]);
			record->filp_pages[idx] = NULL;
		}
	...
	if (record->preread_filpp) {
		free_page((unsigned long)record->preread_filpp);
		record->preread_filpp = NULL;
	}
}
	...
	unsigned count)
{
	...
	if (task->tgid != g_collector->monitor->owner.pid) {
		spin_unlock(&g_collector->lock);
		return;
	}
