...
static int row_be_expire_adjust(struct row_data *rd)
{
	...
	for (i = start_idx; i < end_idx; i++) {
		...
		expire_time = jiffies_to_msecs(temp_jiffies - check_req->fifo_time);
	...
	if (expire_number <= 0)
		...
		pr_crit("ROW_LOG:max expire time:%u in Q%d(%d-%d-%d-%d-%d)!!!\n", max_expire_time,
			expire_index, rd->row_queues[0].nr_req, rd->row_queues[1].nr_req,
			rd->row_queues[2].nr_req, rd->row_queues[3].nr_req, rd->row_queues[4].nr_req);
	return expire_index;
}
...
/*
 * row_get_current() - Get the current running thread
 */
static struct task_struct *row_get_current(void)
{
	return current;
}
