...
#endif
#include "sound/msm89xx_registers_table.h"
...
static struct snd_soc_codec *analog_registered_codec = NULL;
static struct snd_soc_codec *digital_registered_codec = NULL;
#define analog_register_flag  false
#define digital_register_flag  true
...
Others:
*************************************************/
void hw_get_registered_codec(struct snd_soc_codec *codec, bool is_digital_codec)
{
    if ( is_digital_codec ) {
        digital_registered_codec = codec;
    } else {
        analog_registered_codec = codec;
    }
}
EXPORT_SYMBOL_GPL(hw_get_registered_codec);

/*************************************************
Function:       soc_codec_dump_show
...
Others:
*************************************************/
static ssize_t soc_codec_dump_show(struct snd_soc_codec *codec, char *buf, size_t buf_size, bool is_digital_codec)
{
    int reg = 0;
    int reg_max_size = 0;
    int ret = 0;
    int len = WCD_CDC_DUMP_LENGTH;
    ...
    if (is_digital_codec) {
        reg = MSM89XX_CDC_CORE_REG_BASE;
        reg_max_size = MSM89XX_CDC_CORE_CACHE_SIZE;
    } else {
        reg = MSM89XX_PMIC_BASE;
        reg_max_size = MSM89XX_PMIC_CDC_CACHE_SIZE;
    }

    for (reg = 0; reg < reg_max_size; reg ++) {
        if (!is_digital_codec && !msm89xx_analog_codec_readable_reg(reg))
            continue;
        else if (is_digital_codec && !msm89xx_digital_codec_readable_reg(reg))
            continue;

        if (buf_size + len >= count)
            break;
        ...
        /* prepare the buffer */
        snprintf(tmpbuf, len + 1, "%.*x: %s\n", WCD_CDC_DUMP_WORD_SIZE, reg, regbuf);
        /* copy it back to the caller without the '\0' */
        memcpy(buf + buf_size, tmpbuf, len);
...
static ssize_t audio_codec_dump_show(__attribute__((unused))struct device_driver *driver, char *buf)
{
    ...
    struct snd_soc_codec *codec = analog_registered_codec;
    ...
    for (reg = 0; reg < codec->driver->reg_cache_size; reg ++) {
        if (!codec->readable_register(codec, reg))
            continue;
...
#else
    total = soc_codec_dump_show(analog_registered_codec, buf, total, analog_register_flag);
    total = soc_codec_dump_show(digital_registered_codec, buf, total, digital_register_flag);
#endif
