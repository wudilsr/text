...
extern VOS_STATUS drv_auto_nvbin_load(const struct firmware **fw,struct device *dev);
#endif
...
#ifdef CONFIG_HUAWEI_WIFI
   status = drv_auto_nvbin_load(&fw, pHddCtx->parent_dev);
   if(status)
   {
...
VOS_STATUS hdd_set_sme_config( hdd_context_t *pHddCtx )
{
   ...
   pr_info("wlan: enter hdd_set_sme_config\n");
   hdd_config_t *pConfig = pHddCtx->cfg_ini;
   ...
   pr_info("wlan: exit hdd_set_sme_config\n");
   return status;
}
