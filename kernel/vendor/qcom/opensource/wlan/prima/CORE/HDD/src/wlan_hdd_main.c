...
extern int __init hw_buildin_module_init (void);
...
void hdd_dump_concurrency_info(hdd_context_t *pHddCtx)
{
...
#ifdef CONFIG_HUAWEI_WIFI
   hddLog(VOS_TRACE_LEVEL_INFO, "wlan(%d) " HW_MAC_ADDRESS_STR " %s",
                staChannel, HW_MAC_ADDR_ARRAY(staBssid), ccMode);
#else
   ...
   if (p2pChannel > 0) {
#ifdef CONFIG_HUAWEI_WIFI
       hddLog(VOS_TRACE_LEVEL_ERROR, "p2p-%s(%d) " HW_MAC_ADDRESS_STR,
                     p2pMode, p2pChannel, HW_MAC_ADDR_ARRAY(p2pBssid));
#else
   ...
   if (apChannel > 0) {
#ifdef CONFIG_HUAWEI_WIFI
       hddLog(VOS_TRACE_LEVEL_ERROR, "AP(%d) " HW_MAC_ADDRESS_STR,
                     apChannel, HW_MAC_ADDR_ARRAY(apBssid));
#else
...
#ifdef CONFIG_HUAWEI_WIFI
extern char proc_mac_address[MAC_ADDRESS_MAX_LEN];
#endif
/**---------------------------------------------------------------------------
...
static VOS_STATUS hdd_update_config_from_nv(hdd_context_t* pHddCtx)
{
...
#ifdef CONFIG_HUAWEI_WIFI
   ret = memcpy((v_U8_t *)&macFromNV[0].bytes[0],proc_mac_address,WLAN_MAC_LEN);
   ...
   {
   pr_err("hdd_update_config_from_nv get MAC from proc_mac_address failed!\n");
#endif
