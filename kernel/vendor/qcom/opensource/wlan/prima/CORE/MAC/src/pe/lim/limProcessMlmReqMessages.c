...
limProcessMlmAssocReq(tpAniSirGlobal pMac, tANI_U32 *pMsgBuf)
{
   ...
   // tANI_U8                  sessionId;
    ...
    limLog(pMac, LOGE,FL("Process Assoc Req on sessionID %d Systemrole %d"
...
limProcessJoinFailureTimeout(tpAniSirGlobal pMac)
{
    ...
    if (psessionEntry->limMlmState == eLIM_MLM_WT_JOIN_BEACON_STATE)
    {
        ...
        /* ignore join timeout, change the state to success */
        ...
        /* ignore join timeout, change state eLIM_MLM_IDLE_STATE to eLIM_MLM_JOINED_STATE */
        PELOGE(limLog(pMac, LOGE,  FL(" Change limMlmState to eLIM_MLM_JOINED_STATE"));)
        psessionEntry->limMlmState = eLIM_MLM_JOINED_STATE;
        MTRACE(macTrace(pMac, TRACE_CODE_MLM_STATE, psessionEntry->peSessionId, psessionEntry->limMlmState));
        /**
         * ignore join timeout,so don't set eSIR_LINK_IDLE_STATE,or can't receive Auth frame
         */
