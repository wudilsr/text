...
static enum msm_flash_lock_status msm_sub_flash_status = MSM_FLASH_STATUS_UNLOCK;
				...
				enum led_brightness value)
{
	...
	if (MSM_CAMERA_LED_TORCH_POWER_NORMAL == (enum msm_falsh_power_statue)value) {
		msm_sub_flash_status = MSM_FLASH_STATUS_UNLOCK;
		pr_info("power saving [sub_flash] is normal\n");
	}
	else if (MSM_CAMERA_LED_TORCH_POWER_LOW == (enum msm_falsh_power_statue)value) {
		msm_sub_flash_status = MSM_FLASH_STATUS_LOCKED;
		pr_info("power saving lock the [sub_flash] \n");
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	flash_ctrl->last_flash_current = 0;
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if(flash_data->flash_current[0] == flash_ctrl->last_flash_current){
		CDBG("flash_current is not changed: %d", flash_data->flash_current[0]);
		return 0;
	}
	...
	CDBG("Exit\n");
	flash_ctrl->last_flash_current = flash_data->flash_current[0];
	return 0;
}
	...
	void *argp)
{
	...
	if (((CFG_FLASH_LOW == flash_data->cfg_type) || (CFG_FLASH_HIGH == flash_data->cfg_type)) && (MSM_FLASH_STATUS_LOCKED == msm_sub_flash_status))
	{
		pr_err("Low power! [sub_flash] cannot work for flash type:%d \n",flash_data->cfg_type);
...
MODULE_DESCRIPTION("MSM SUBFLASH");
