...
int mmc_ffu_restart(struct mmc_card *card)
{
	...
	if (err)
	{
		pr_err("%s: powering down failed %d.\n", __func__ , err);
		goto exit;
	}
...
exit_normal:
	...
	ret = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
		...
		card->ext_csd.generic_cmd6_time);
		pr_err("mmc_switch_ret = %d\n ---yx---", ret);
		if (ret)
		err = ret;

exit:
	pr_err("mmc_ffu_install_download_err = %d\n ---yx---", err);
	return err;
}
