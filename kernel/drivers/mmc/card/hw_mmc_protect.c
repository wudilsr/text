...
/* for 4.9 kernel,  keep same to the struct in block.c */
struct mmc_blk_data {
	spinlock_t	lock;
...
int mmc_blk_cmdq_switch_hw(struct mmc_card *card,
...
static int mmc_switch_out_rpmb(u8 *ext_csd, struct mmc_card *card)
{
	u8 part_config;
	...
	part_config = ext_csd[EXT_CSD_PART_CONFIG];
	pr_err("part_config = %d\n", part_config);
	if ((card->ext_csd.rev >= 3) && 
				((part_config & EXT_CSD_PART_CONFIG_ACC_MASK) == EXT_CSD_PART_CONFIG_ACC_RPMB)) {
		part_config &= ~EXT_CSD_PART_CONFIG_ACC_MASK;
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONFIG, part_config, card->ext_csd.part_time);
		if (err ) {
			pr_err("%s: err %d mmc_switch.\n", mmc_hostname(card->host), err);
			return err;
		}
		card->ext_csd.part_config = part_config;
		card->part_curr = part_config & EXT_CSD_PART_CONFIG_ACC_MASK;
		main_md = mmc_get_drvdata(card);
		if (main_md) {
			main_md->part_curr = card->part_curr;
		}
	}
	return 0;
}
...
static int hw_mmc_do_get_write_protection_info(struct gendisk *disk, struct hd_struct *part)
{
	...
	if (mmc_switch_out_rpmb(card->cached_ext_csd, card))
		goto out;
...
static int hw_mmc_do_set_write_protection(struct gendisk *disk, struct hd_struct *part)
{
	...
	if(sector_size % wp_group_size) {
		...
		sector_size = round_down(sector_size, wp_group_size);
	...
	if(sector_start_addr % wp_group_size) {
		...
		sector_start_addr  = sector_start_addr / wp_group_size * wp_group_size + wp_group_size;
	...
	if (mmc_switch_out_rpmb(ext_csd, card))
		goto out;
...
int mmc_blk_ioctl_huawei_ext(struct block_device *bdev, fmode_t mode,
...
EXPORT_SYMBOL(mmc_blk_ioctl_huawei_ext);
