...
unsigned int mmc_get_sd_speed(struct mmc_card* card)
{
    unsigned int speed = 0;
    switch(card->ssr.speed_class)
	{
        case 0x00:
            speed = 0;
            break;
...
int mmc_get_rw_status(int status)
{
    ...
    if(status >= MMC_BLK_CMD_ERR && status <= MMC_BLK_ECC_ERR)
    {
        ...
        if(err_num >= MAX_ERR_TIMES)
        {
            ...
            err_num = 0;
#ifdef CONFIG_HUAWEI_SDCARD_DSM
		dsm_sdcard_report(DSM_SDCARD_STATUS_BLK_RW_CHECK_ERR,DSM_SDCARD_BLK_RW_CHECK_ERR);
#endif
        }
        else
        {
...
===========================================================================*/
unsigned int mmc_calculate_ioworkload_and_rwspeed(unsigned long long time, struct request *rqc, struct gendisk *disk, struct mmc_card *card)
{
   ...
   if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
   {
       ...
       printk("SD_HEALTH_DETECT:mmcqd/1 rw time:%lld ns,write time:%lld ns,read time:%lld ns\n",t_usage,mmcqd_t_usage_wr,mmcqd_t_usage_rd);
       /* worload < 0.01*/
       if(t_period > t_usage*100)// io workload < 1%
       {
          printk("SD_HEALTH_DETECT:mmcqd/1 workload < 1%%, duty %lld, period %lld, req_cnt=%d\n",t_usage,t_period,mmcqd_rq_count);
       }
       else
       {
          ...
          printk("SD_HEALTH_DETECT:mmcqd/1 rw_workload = %d%%, w_workload = %d%%, duty %lld, period %lld00, req_cnt=%d\n",t_percent, t_percent_wr, t_usage, t_period,mmcqd_rq_count);
       }
       ...
       if(mmcqd_t_usage_wr)
       {
          ...
          if(mmcqd_t_usage_wr)  //if >=1ms
          {
              ...
              if((t_percent_wr >= SD_IO_BUSY) &&(perf_meter < LOW_SPEED_WARTING_VALUE))
              {
                 ...
                 if(wr_speed_abnor_times == MAX_WRITE_SPEED_ABNOR_TIMES)
                 {
                     ...
                     printk(KERN_ERR "SD_HEALTH_DETECT:mmcqd/1:sd_spec:%d,the card wr_speed is lower than spec\n",mmc_get_sd_speed(card));
#ifdef CONFIG_HUAWEI_SDCARD_DSM
                     dsm_sdcard_report(DSM_SDCARD_STATUS_BLK_WR_SPEED_ERR,DSM_SDCARD_BLK_WR_SPEED_ERR);
#endif 
		}
              }
              else
          ...
          else
          {
              ...
              printk("SD_HEALTH_DETECT:mmcqd/1 Write Throughput=%d KB/s, size: %d bytes, time:%lld ms\n",perf_meter,mmcqd_rq_size_wr,mmcqd_t_usage_wr);
       }
       else
       {
           wr_speed_abnor_times = 0;
       ...
       if(mmcqd_t_usage_rd)
       {
          ...
          if(mmcqd_t_usage_rd)
          {
             ...
             printk("SD_HEALTH_DETECT:mmcqd/1 Read Throughput=%d kB/s, size: %d bytes, time:%lld ms\n",perf_meter,mmcqd_rq_size_rd,mmcqd_t_usage_rd);
          }
       }
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
	dsm_sdcard_report(DSM_SDCARD_STATUS_RO_ERR, DSM_SDCARD_RO_ERR);
#endif
      return 0;
   }
