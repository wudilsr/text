...
#ifdef CONFIG_HUAWEI_QCOM_MMC
		if((!strcmp(mmc_hostname(msm_host->mmc), "mmc1")) & (msm_host->pltfm_init_done != true))
...
static int sdhci_msm_set_gpio_info(struct sdhci_msm_pltfm_data *pdata)
{
	...
	if(!np)
	{
		...
		pdata->caps2 |= MMC_CAP2_CD_ACTIVE_HIGH;
		ret = 1;
		pr_info("%s (): np is NULL, default is MMC_CAP2_CD_ACTIVE_HIGH \n",__func__ );
		return ret;
	}
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	msm_host->mmc->mmc_enter_ioctl_multi_cmd = 0;
#endif
	...
	if (ret) {
 	...
 	if (gpio_is_valid(msm_host->pdata->status_gpio) &&
		(msm_host->mmc->slot.cd_irq >= 0)){
			ret = enable_irq_wake(msm_host->mmc->slot.cd_irq);
			dev_err(&pdev->dev, "%s: mmc1: enable_irq_wake ret=%d .\n", __func__,ret);
		}
