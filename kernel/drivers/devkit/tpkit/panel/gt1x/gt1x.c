...
static void gt1x_pinctr_int_ouput_high(void);
...
static int gt1x_set_voltage(void)
{
	int ret = 0;
	struct gt1x_ts_data *ts = gt1x_ts;

	if(GT1X_POWER_CONTRL_BY_SELF == ts->power_self_ctrl 
			&& GT1X_POWER_BY_LDO == ts->vci_power_type 
			&& GT1X_POWER_BY_LDO == ts->vddio_power_type){
		ret = regulator_set_voltage(ts->vci,
					ts->dev_data->regulator_ctr.vci_value,
					ts->dev_data->regulator_ctr.vci_value);
		if (ret < 0) {
			TS_LOG_ERR("%s : Failed to set vci voltage:%d\n", __func__, ret);
			return -EINVAL;
		}
		TS_LOG_INFO("%s : set vci voltage OK\n", __func__);

		if(ts->dev_data->regulator_ctr.need_set_vddio_value){
			ret = regulator_set_voltage(ts->vddio,
						ts->dev_data->regulator_ctr.vddio_value,
						ts->dev_data->regulator_ctr.vddio_value);
			if (ret < 0) {
				TS_LOG_ERR("%s: Failed to set vddio voltage:%d\n", __func__, ret);
				ret = -EINVAL;
				goto exit;
			}
				TS_LOG_INFO("%s:set vddio voltage OK\n", __func__);
		}else{
			TS_LOG_INFO("%s:no need set vddio voltage\n", __func__);
		}
	}
...
static int gt1x_get_regulators(void)
{
	int ret = 0;
	struct gt1x_ts_data *ts = gt1x_ts;
	
	if(GT1X_POWER_CONTRL_BY_SELF != ts->power_self_ctrl){
		TS_LOG_INFO("%s: power contrl by lcd\n", __func__);
		return NO_ERR;
	}

	if(GT1X_POWER_BY_LDO == ts->vci_power_type
			&& GT1X_POWER_BY_LDO == ts->vddio_power_type){
		ts->vci= regulator_get(&ts->pdev->dev, "vci");
		if (IS_ERR(ts->vci)) {
			ret = PTR_ERR(ts->vci);
			TS_LOG_ERR("%s:Regulator get of vdd_ana failed\n", __func__);
			return -EINVAL;
		}
		TS_LOG_INFO("%s:Regulator get vci is OK\n", __func__);

		ts->vddio = regulator_get(&ts->pdev->dev, "vddio");
		if (IS_ERR(ts->vddio)) {
			ret = PTR_ERR(ts->vddio);
			TS_LOG_ERR("%s:Regulator get of vddio failed\n", __func__);
			goto exit;
		}
		TS_LOG_INFO("%s:Regulator get vddio is OK\n", __func__);
	} else {
		TS_LOG_ERR("%s:No support power type\n", __func__);
		ret = -EINVAL;
	}
...
exit:
	if(GT1X_POWER_BY_LDO == ts->vci_power_type){
		regulator_put(ts->vci);
		ts->vci = NULL;
	}
	return ret;
}
static void gt1x_put_regulators(void)
{
	struct gt1x_ts_data *ts = gt1x_ts;
 ...
 */
static int gt1x_power_switch(int on)
{
	...
	udelay(100);
	if (GT1X_POWER_CONTRL_BY_SELF != ts->power_self_ctrl) {
		TS_LOG_DEBUG("%s: power contrl by lcd\n", __func__);
		return ret;
	}

	if (on) {
		TS_LOG_INFO("%s:GTP power ON\n", __func__);
		if(GT1X_POWER_BY_LDO == ts->vci_power_type
				&& GT1X_POWER_BY_LDO == ts->vddio_power_type){
			ret = regulator_enable(ts->vddio);
			if (ret < 0) {
				TS_LOG_ERR("%s:failed to enable regulator vddio\n", __func__);
				ret = -EINVAL;
				goto exit;
			}
			udelay(2);
			ret = regulator_enable(ts->vci);
			if (ret < 0) {
				TS_LOG_ERR("%s: failed to enable regulator vci\n", __func__);
				ret = -EINVAL;
				goto exit;
			}
		}
		...
		TS_LOG_INFO("%s:GTP power OFF\n", __func__);
		if (GT1X_POWER_BY_LDO == ts->vci_power_type
				&& GT1X_POWER_BY_LDO == ts->vddio_power_type) {
			ret = regulator_disable(ts->vddio);
			if (ret < 0) {
			...
			udelay(2);
			ret = regulator_disable(ts->vci);
			if (ret < 0) {
				TS_LOG_INFO("%s:failed to disable regulator vci\n", __func__);
				ret = -EINVAL;
				goto exit;
			}  
		}
		udelay(2);
		gt1x_power_off_gpio_set();// continue ... add liangqi
	}
...
static int gt1x_pinctrl_init(void)
{
	...
	else
	{
		...
		ts->pins_int_high = pinctrl_lookup_state(ts->pinctrl, "ts_int_high");
		if (IS_ERR_OR_NULL(ts->pins_int_high)) {
			TS_LOG_ERR("%s: Pin state[int_high] not found\n", __func__);
			ret = PTR_ERR(ts->pins_int_high);
			goto exit_put;
		}

		ts->pins_int_low = pinctrl_lookup_state(ts->pinctrl, "ts_int_low");
		if (IS_ERR_OR_NULL(ts->pins_int_low)) {
			TS_LOG_ERR("%s: Pin state[int_low] not found\n", __func__);
			ret = PTR_ERR(ts->pins_int_low);
			goto exit_put;
		}
	}
	return ret;
exit_put:
	if(ts->pinctrl){
		...
		ts->pins_int_high = NULL;
		ts->pins_int_low = NULL;
	}
	return ret;
}
...
/**
 * gt1x_pinctr_int_ouput_high - set irq pin to high-level state.
 */
static void gt1x_pinctr_int_ouput_high(void)
{
	int ret = 0;
	struct gt1x_ts_data *ts = gt1x_ts;
	if (ts->pinctrl && ts->pins_int_high) {
		ret = pinctrl_select_state(ts->pinctrl, ts->pins_int_high);
		if (ret < 0)
			TS_LOG_ERR("%s:Set irq pin to high-level state failed:%d .\n", __func__, ret);
	}
	return;
}
...
/**
 * gt1x_get_tp_color - get tp_color.
 * @ts: gt1x ts dat
 * Return 0-succeed
 */
static int gt1x_get_tp_color(struct gt1x_ts_data *ts)
{
	u8 buf[TP_COLOR_DATA_LENGTH] = {0xFF};
	int ret = 0,i = 0;
	char buf_total = 0;
	ret = ts->ops.i2c_read(TP_COLOR_REG, buf,TP_COLOR_DATA_LENGTH);
	if (ret != 0) {
		TS_LOG_ERR("%s: I2C error,read reg 0x99a4 Failed %d\n", __func__, ret);
	} else {
		for(i = 0;i < TP_COLOR_DATA_LENGTH;i++) {
			buf_total += buf[i];
		}
		TS_LOG_INFO("%s: read tp_color succ,checksum buf_total = %d\n", __func__, buf_total);
		if(TP_COLOR_CHECKSUM == buf_total) {
			cypress_ts_kit_color[0] = buf[0];
		} else {
			TS_LOG_ERR("%s: read reg 0x99a4,Checksum Failed\n", __func__);
		}
	}
	TS_LOG_INFO("%s: tp_color = %x\n", __func__, buf[0] );
	return ret;
}
...
static int gt1x_prepar_parse_dts(struct ts_kit_platform_data *pdata)
{
	...
	ret = of_property_read_u32(pdata->chip_data->cnode , GT1X_SLAVE_ADDR ,
								&value);
	if (ret) {
	...
	ret = of_property_read_u32(pdata->chip_data->cnode, GTP_NEED_SET_VDDIO_VALUE, &value);
	if (ret) {
		value = 0;
	...
	if (ret) {
		chip_data->regulator_ctr.vci_value = 2800000; /* 2.8v */
		TS_LOG_INFO("%s : Use default vci voltage:2.8v\n", __func__);
	}
	ret = of_property_read_u32(pdata->chip_data->cnode, GTP_VDDIO_LDO_VALUE,
						&chip_data->regulator_ctr.vddio_value);
	if (ret) {
		chip_data->regulator_ctr.vddio_value = 1800000; /* 1.8v */
...
static int gt1x_parse_dts(void)
{
	...
	ret = of_property_read_u32(device, GTP_ROI_SUPPORTED, &value);
	if (ret) {
		TS_LOG_INFO("%s: roi_supported use default value\n", __func__);
		value = 0;
	}
	...
	ret = of_property_read_u32(device, GTP_HOLSTER_SUPPORTED, &value);
	if (ret) {
		TS_LOG_INFO("%s: holster_supported use default value\n", __func__);
		value = 0;
	}
	chip_data->ts_platform_data->feature_info.holster_info.holster_supported = (u8)value;

	ret = of_property_read_u32(device, GTP_GLOVE_SUPPORTED, &value);
	if (ret) {
		value = 0;
		TS_LOG_INFO("%s: glove_supported use default value\n", __func__);
	}
	chip_data->ts_platform_data->feature_info.glove_info.glove_supported = (u8)value;
	...
	ret = of_property_read_u32(device, "use_lcdkit_power_notify", &value);
	if (ret) {
		value = 0;
		TS_LOG_INFO("%s: easy_wakeup_supported use default value\n", __func__);
	}
	ts->dev_data->use_lcdkit_power_notify= (u8)value;
	...
	ret = of_property_read_string(device, GTP_TP_TEST_TYPE,(const char**)&tmp_buff);
	if (ret || tmp_buff == NULL) {
		TS_LOG_INFO("%s: get device tp_test_type not exit,use default value\n", __func__);
		strncpy(chip_data->tp_test_type,
			"Normalize_type:judge_last_result",
			TS_CAP_TEST_TYPE_LEN);
	} else {
		strncpy(chip_data->tp_test_type,tmp_buff,TS_CAP_TEST_TYPE_LEN);
		TS_LOG_INFO("%s: tp_test_type is %s \n", __func__,chip_data->tp_test_type);
	}
	ret = of_property_read_u32(device, GTP_CHARGER_SUPPORT, &value);
	if (ret) {
		value = 0;
		TS_LOG_INFO("%s: charger_supported will use default value\n", __func__);
	}
	chip_data->ts_platform_data->feature_info.charger_info.charger_supported =(u8)value;

	ret = of_property_read_u32(device, GTP_TOUCH_SWITH_FLAG, &chip_data->touch_switch_flag);
	if (ret) {
		TS_LOG_INFO("%s get touch_switch_flag from dts failed, use default(0).\n", __func__);
		chip_data->touch_switch_flag = 0;
	}

	TS_LOG_INFO("%s: irq_config=%d,algo_id=%d,project_id=%s,config_parm_flag=%d,roi_supported=%d,"
		"holster_supported=%d,glove_supported=%d, easy_wakeup_supported=%d,create_project_id=%d,"
		"tools_support=%d,charger_supported=%d,ic_type=%d,chip_name=%s,touch_switch_flag=%d\n",
		__func__, chip_data->irq_config, chip_data->algo_id,ts->project_id,ts->config_flag,
		...
		ts->easy_wakeup_supported,ts->create_project_id_supported,ts->tools_support,
		chip_data->ts_platform_data->feature_info.charger_info.charger_supported,
		ts->ic_type,ts->chip_name,chip_data->touch_switch_flag);
...
static int gt1x_parse_specific_dts(void)
{
	...
	ret = of_property_read_u32(device, GT1X_PROVIDE_PANEL_ID_SUPPORT, &value);
	if (!ret)
		ts->dev_data->provide_panel_id_support = value;

	TS_LOG_INFO("%s: x_max_mt=%d,y_max_mt=%d,panel_id=%d,vendor_name=%s,provide_panel_id_support=%d,project_id=%s\n", 
		__func__, ts->max_x, ts->max_y,ts->dev_data->ts_platform_data->panel_id,ts->dev_data->module_name,
		ts->dev_data->provide_panel_id_support,ts->project_id);
	return NO_ERR;
}
...
static int gt1x_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	if (ret< 0) {
		TS_LOG_ERR("%s, failed to set voltage, ret = %d\n", __func__, ret);
...
static int gt1x_chip_init(void)
{
	...
	ret = gt1x_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS,
			reg_val, 1);
	...
	ts_kit_anti_false_touch_param_achieve(gt1x_ts->dev_data);
	...
	if(ts->support_get_tp_color) {
		gt1x_get_tp_color(ts);
		TS_LOG_INFO("%s: cypress_ts_kit_color %x\n", __func__,cypress_ts_kit_color[0]);
	} else {
		TS_LOG_INFO("%s: tp color Unsupport,support_get_tp_color = [%d]\n", __func__,ts->support_get_tp_color);
	}
	TS_LOG_INFO("%s: end\n", __func__);
	return NO_ERR;
}
...
void gt1x_reset_select_addr(void)
{
	TS_LOG_INFO("%s: Start to make chip-selection for 0x14 slave sddress!\n", __func__);
...
struct ts_device_ops ts_gt1x_ops = {
	...
	.chip_get_capacitance_test_type =
		gt1x_chip_get_capacitance_test_type,
