...
#define PAD0_RAW_PART_PATH "/dev/block/bootdevice/by-name/pad0"
...
static struct delayed_work write_offset_delay_work;  /* work to write offset to raw part pad0 */
...
static long write_data_to_file(int fd, char *buf, size_t buf_size)
{
	...
	{
		bytes_this_time = sys_write(fd, ptemp, bytes_total_to_rw);
...
write_offset_to_raw_part(struct work_struct *work){
	...
	ret = write_emmc_raw_part(PAD0_RAW_PART_PATH, RTC_PART_OFFSET, (char*)&offset_info, sizeof(Offset_Info));
	if (0 != ret)
	{
		pr_err("write [%s] failed ret: %d!\n", PAD0_RAW_PART_PATH, ret);
	}
}
...
int huawei_rtc_remove(void){

	cancel_work_sync(&write_offset_delay_work);
	flush_workqueue(rtc_off_work_queue);
