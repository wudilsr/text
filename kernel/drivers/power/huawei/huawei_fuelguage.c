...
extern void fg_dump_register(char* reg);
extern void fg_get_register_head(char * reg);
#endif
#define FG_LOG_STR_LEN      50
static ssize_t fuelguage_sysfs_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 9, 0))
		fg_get_register_head(di->sysfs_data.reg_head);
#else
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 9, 0))
		fg_dump_register(di->sysfs_data.reg_value);
#else
				     ...
				     const char *buf, size_t count)
{
    ...
    if (!dev || !attr || !buf) {
        pr_err("%s: invaldi param, fatal error\n", __func__);
        return -EINVAL;
    }
	...
	case FUELGUAGE_SYSFS_TEST:
        /* valid input value must be less than 10 */
		if ((kstrtol(buf, INPUT_NUMBER_BASE, &val) < 0) || val > 10) {
			return -EINVAL;
		}
...
static int fuel_guage_probe(struct platform_device *pdev)
{
	...
	const  char *fuelguage_type_string = NULL;
	int ret = 0;
