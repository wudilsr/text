...
#else
	if (chip->batt_psy && chip->batt_psy->desc->get_property) {
#endif
...
static  struct power_supply_desc dsm_psy_desc = {
	.name = "dsm",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.get_property = chg_dsm_get_property,
	.set_property = chg_dsm_set_property,
	.external_power_changed =  chg_dsm_external_power_changed,
	.property_is_writeable = NULL,
	.num_properties = 0,
	.properties =0,
};

static int hw_init_dsm_psy(struct hw_dsm_charger_info *chip)
{
	struct power_supply_config dsm_cfg = {};
	int rc = 0;

	dsm_cfg.drv_data = chip;
	dsm_cfg.of_node = chip->dev->of_node;

	chip->dsm_psy = power_supply_register(chip->dev,
						  (const struct power_supply_desc*) (&dsm_psy_desc),
						   &dsm_cfg);

	if (IS_ERR(chip->dsm_psy)) {
		pr_err("Couldn't register dsm power supply\n");
		return PTR_ERR(chip->dsm_psy);
	}
