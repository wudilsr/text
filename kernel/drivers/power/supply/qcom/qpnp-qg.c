...
static struct qpnp_qg *global_fg_chip = NULL;
...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	prop.intval = chip->bp.float_volt_uv/1000;
	...
	prop.intval = chip->bp.fastchg_curr_ma*MV_TURNTO_UV ;
	rc = power_supply_set_property(chip->batt_psy,
...
static void qg_reset_learned_cc(struct qpnp_qg *chip, int clear)
{
       int64_t temp = 0;
       int rc = 0;
       ...
       if (!rc)
		chip->cl->learned_cap_uah = temp;
       qg_store_learned_capacity(chip, chip->cl->learned_cap_uah);
}
...
void fg_dump_register(char* reg)
{
	int rc = 0;
	u32 batt_id = 0,charge_count=0;
	int batt_soc = 0, resistance = 0,rbat_conn_mohm = 0;
	...
	if (!reg) {
		pr_err("reg is not init ready\n");
	}

	memset(reg, 0, CHARGELOG_SIZE);

	/* add 14 item info, the head is chars */
	rbat_conn_mohm = global_fg_chip->dt.rbat_conn_mohm;

	rc = qg_get_battery_capacity(global_fg_chip, &batt_soc);
	if (rc < 0) {
		pr_err("failed to get ESR, rc=%d\n", rc);
	}	
	rc = get_batt_id_ohm(global_fg_chip,&batt_id);
	if (rc < 0) {
		pr_err("failed to get batt_id, rc=%d\n", rc);
	}	
	resistance = global_fg_chip->batt_id_ohm;
	charge_count = global_fg_chip->charge_counter_uah;


	snprintf(reg, MAX_SIZE, "%-17d %-11d %-10d %-15d %-14d ",
			 rbat_conn_mohm,batt_soc, batt_id, resistance, charge_count);
}

void fg_get_register_head(char * reg)
{
	...
	snprintf(reg, MAX_SIZE, "rbat_conn_mohm    batt_soc    batt_id    resistance    charge_count    ");
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHG_CYCLE_COUNT:
               rc = get_chg_cycle_count(chip->counter, &pval->intval);
               break;
       case POWER_SUPPLY_PROP_RESET_LEARNED_CC:
               pval->intval = 0;
               break;
			...
			unsigned long event, void *data)
{
	...
	if ((strcmp(psy->desc->name, "bk_battery") == 0)
...
static int qg_determine_pon_soc(struct qpnp_qg *chip)
{
	...
	qg_dbg(chip, QG_DEBUG_PON, "Shutdown: Valid=%d SOC=%d OCV=%duV time=%dsecs, time_now=%ldsecs\n",
			...
			rtc_sec);
	pr_info( "Shutdown: Valid=%d SOC=%d OCV=%duV time=%dsecs, time_now=%ldsecs\n",
			shutdown[SDAM_VALID],
			shutdown[SDAM_SOC],
			shutdown[SDAM_OCV_UV],
			shutdown[SDAM_TIME_SEC],
	...
	if (use_pon_ocv == true) {
		...
		if (ocv[S3_LAST_OCV].ocv_raw == FIFO_V_RESET_VAL) {
			if (!ocv[SDAM_PON_OCV].ocv_uv) {
				strlcpy(ocv_type, "S7_PON_SOC", STR_LENGTH);
				...
				strlcpy(ocv_type, "SDAM_PON_SOC", STR_LENGTH);
				ocv_uv = ocv[SDAM_PON_OCV].ocv_uv;
			}
		} else {
			if (ocv[S3_LAST_OCV].ocv_uv >= ocv[S7_PON_OCV].ocv_uv) {
				strlcpy(ocv_type, "S3_LAST_SOC", STR_LENGTH);
