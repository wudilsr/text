...
static int check_sim_status(struct i2c_client *client, struct  pn547_dev *pdev)
{
    ...
    /*write TEST_SWP_CMD UICC2*/
    ret = pn547_i2c_write(pdev, read_config_UICC2, sizeof(read_config_UICC2));
    if (ret < 0) {
        pr_err("%s: TEST_ESE_CMD pn547_i2c_write failed, ret:%d\n", __func__, ret);
        goto failed;
    }
    ...
    /*NTF's format: 6F 3E 02 XX1 XX2 -> "XX1 == 0" means SWP link OK*/
    if (recvBuf[0] == 0x6F && recvBuf[1] == 0x3E && recvBuf[3] == 0x00) {
        pdev->sim_status |= UICC_2_SUPPORT_CARD_EMULATION;
    }
...
/*lint -e529 -esym(529,*)*/
    ...
    if (dmd_no < NFC_DMD_NUMBER_MIN || dmd_no > NFC_DMD_NUMBER_MAX
        ...
        pr_info("%s: para error: %d\n", __func__, dmd_no); /*lint !e960*/
        return -1;
    }

    pr_info("%s: dmd no: %d - %s", __func__, dmd_no, dmd_info); /*lint !e960*/
        ...
        const struct i2c_device_id *id)
{
    ...
    (void)clk_set_rate(nfc_clk, 19200000);
