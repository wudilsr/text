...
next_step:
		...
		if (phase == 3) {
			...
			if (f2fs_encrypted_file(inode)) {
				if(is_platform_hardware_fbe_enabled()) {
					...
					if(err || (inode && (NULL == inode->i_crypt_info))) {
						printk(KERN_ERR "F2FS: fscrypt_get_encryption_info failed, inode->i_ino = %lu, err = %d\n", inode->i_ino, err);
						iput(inode);
						continue;
					}
				}
	...
	if(is_platform_hardware_fbe_enabled() && (gc_type == FG_GC))
	{
		__f2fs_submit_merged_write(sbi, DATA, COLD);
	}
