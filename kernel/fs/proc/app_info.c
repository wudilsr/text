		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	int name_lenth=0,value_lenth=0,buf_len=0;
	int ret=HW_FUNC_RET_FAIL;
	int i = 0;
	bool flag = false;
	char* strtok = NULL;
	char *temp_ptr = NULL;
	...
	if (NULL == buf)
	{
		pr_err("%s: input paramter invalid!\n",__func__);
		return HW_FUNC_RET_FAIL;
	}
	buf_len = strlen(buf);
	AppStr = kmalloc(buf_len+HW_APP_INFO_5_VAL, GFP_KERNEL);
	if (!AppStr)
	{
	...
	memset(AppStr, 0, buf_len+HW_APP_INFO_5_VAL);
	snprintf(AppStr, buf_len + HW_APP_INFO_5_VAL - HW_APP_INFO_1_VAL, "%s", buf);
	strtok = strsep(&AppStr, APP_NAME_VALUE_SPLIT_CHAR);
	if (strtok != NULL)
	{
		name_lenth = strlen(strtok);
		memcpy(AppName,strtok,((name_lenth > (APP_INFO_NAME_LENTH-HW_APP_INFO_1_VAL))?(APP_INFO_NAME_LENTH-HW_APP_INFO_1_VAL):name_lenth));
	}
	else
	{
	...
	if(strtok!=NULL)
	{
		...
		memcpy(AppValue,strtok,((value_lenth > (APP_INFO_VALUE_LENTH-HW_APP_INFO_1_VAL))?(APP_INFO_VALUE_LENTH-HW_APP_INFO_1_VAL):value_lenth));
	}
	else
	{
	...
	for (i = 0; i < SENSOR_MAX; i++)
	{
		...
		{
			strncpy(sensor_type[i].name, AppName, sizeof(AppName) - HW_APP_INFO_1_VAL);
			break;
		}
	}
...
void export_ddr_info(unsigned int ddr_vendor_id,unsigned int ddr_size, unsigned int ddr_type )
{
	...
	if( ddr_size >= DDR_SIZE_MIN && ddr_size <=DDR_SIZE_MAX ) //should be less than 8GB
	{
...
void app_info_print_smem(void)
{
    ...
    unsigned int ddr_size0 = 0;
	unsigned int ddr_size1 = 0;
    ...
    ddr_size = (smem->lpddrID >> HW_RIGHT_SHIFT_8) & HW_MASK;
    ddr_type = (smem->lpddrID >> HW_RIGHT_SHIFT_24) & HW_MASK;
