...
static bool platform_hardware_fbe_enabled;

#define PLATFORM_NAME_MAX_LENGTH    50
static bool is_platform_support_hardware_fbe(void) {

	int ret = 0;
	char *p1 = NULL;
	char *p2 = NULL;
	static char platform_name[PLATFORM_NAME_MAX_LENGTH];

	p1 = strnstr(saved_command_line, "androidboot.chiptype=", strlen(saved_command_line));
	if (p1 == NULL) {
		ret = -1;
		return ret;
	}
	...
	strlcpy(platform_name, p1, ((p2 - p1 + 1) > PLATFORM_NAME_MAX_LENGTH)?
                     PLATFORM_NAME_MAX_LENGTH : (p2 - p1 + 1));

	printk(KERN_ERR "PFK_F2FS: platform_name = %s\n", platform_name);
	if(strnstr(platform_name, "SDM450", PLATFORM_NAME_MAX_LENGTH)) {
		return true;
	} else {
		return false;
	}
...
void pfk_f2fs_deinit(void)
{
	...
	platform_hardware_fbe_enabled = false;
}
...
int __init pfk_f2fs_init(void)
{
	...
	if((is_platform_support_hardware_fbe()) && (IS_ENABLED(CONFIG_F2FS_FS_ICE_ENCRYPTION)))
		platform_hardware_fbe_enabled = true;
	else
		platform_hardware_fbe_enabled = false;
	pr_info("PFK platform_hardware_fbe_enabled is %s\n", platform_hardware_fbe_enabled ? "true" : "false");
	return 0;
}

bool is_platform_hardware_fbe_enabled(void) {
	return platform_hardware_fbe_enabled;
}
